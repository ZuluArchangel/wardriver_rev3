...
void clear_display()
{
  // Clears the LCD and resets the cursor. This was editied to fill with black
  ...
  if (filepath == "/B.bin")
  {
    ...
    while (!update_ready)
    {
      ...
      if (ready_failures > 99)
      {
        ...
        clear_display();
        tft.println("FAILURE");
        tft.println("Update B manually!");
        tft.println("OTA not supported");
        delay(10000);
        return false;
      }
    }
    ...
    clear_display();
    tft.println("Completing install");
    tft.println("Please wait");
    tft.println("DO NOT POWER OFF");
    int tocounter = 0;
    ...
    while (!did_update)
    {
      ...
      {
        tft.println("Finalizing..");
      }
      tft.println("DO NOT POWER OFF");
      tft.print("Count:");
      tocounter++;
      tft.println(tocounter);
      if (buff.indexOf(actual_hash) > -1)
      {
...
void boot_config()
{
  ...
  if (con_ssid != "" || fb_ssid != "")
  {
    ...
    if (WiFi.status() == WL_CONNECTED || created_network == true)
    {
      ...
      if (!created_network)
      {
        tft.println("Attempting NTP sync");
      ...
      while (WiFi.status() == WL_CONNECTED || created_network == true)
      {
        ...
        {
          tft.println(device_type_string());
          tft.println(WiFi.localIP());
        }
        tft.print((disconnectat - millis()) / 1000);
        tft.println("s until boot");
        if (update_available)
        {
                    ...
                    while (1)
                    {
                      if (client.available())
                      {
                        ...
                        if (bytesin % 4096 == 0)
                        {
                          clear_display();
                          tft.println("Firmware upload");
                          tft.print(bytesin / 1024);
                          tft.println("kb received");
                        }
                      }
...
void setup()
{
  ...
  tft.initR(INITR_GREENTAB); // or the appropriate INITR_ for your display
  tft.fillScreen(ST77XX_BLACK);
  ...
  {
    clear_display();
    tft.println("Unexpected reset");
    tft.print("Code ");
    tft.println(reset_reason);
    tft.print("Version ");
    tft.println(VERSION);
    delay(4000);
  }
  ...
  while (!filewriter)
  {
    ...
    if (!filewriter)
    {
      ...
      clear_display();
      tft.println("SD File open failed!");
...
void lcd_show_stats()
{
  ...
  {
    tft.print("HDOP:");
    tft.print(((float)nmea.getHDOP() / 10));
    tft.print(" Sats:");
    tft.print(nmea.getNumSatellites());
    tft.println(nmea.getNavSystem());
  }
  else
  {
    tft.print("No GPS: ");
    ...
    if (gsm_loc.acc > 0)
    {
      tft.println("GSM pos OK");
    }
    else
    {
      tft.println("No GSM pos");
    }
  }
#define B_RESET_SEARCH_TIME 20000
  if (b_working && millis() - side_b_reset_millis > B_RESET_SEARCH_TIME)
  {
    ...
    tft.print(" GSM:");
    tft.println(disp_gsm_count);
  }
  else
  {
    if (millis() - side_b_reset_millis > B_RESET_SEARCH_TIME)
    {
      tft.println("ESP-B NO DATA");
    }
    else
    {
      tft.println("ESP-B RESET");
    }
  }
  tft.println(dt_string());
  if (gsm_count > 0)
  {
